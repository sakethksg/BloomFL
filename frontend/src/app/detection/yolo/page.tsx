...
export default function YOLODetectionPage() {
  ...
  const fetchCheckpoints = () => {
    setCkptLoading(true);
    api.inference.checkpoints("yolo")
      .then((list) => {
        // Filter to only YOLO models
        const yoloModels = list.filter((c) => c.detection_type === "yolo");
        setCheckpoints(yoloModels);
        const existing = yoloModels.filter((c) => c.exists).map((c) => c.path);
              ...
              <div className="space-y-6 animate-in fade-in duration-500">
                ...
                {loading ? (
                   <div className="h-[400px] rounded-2xl border-2 border-dashed border-muted flex flex-col items-center justify-center gap-4 text-muted-foreground bg-muted/10">
                      ...
                      <p className="font-medium">Running YOLO inference...</p>
                   </div>
                ) : (
                  <>
                    {/* ── Single Mode View ── */}
